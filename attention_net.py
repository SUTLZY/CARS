...
class Encoder_Context(nn.Module):
    
    def __init__(self, input_dim, hidden_dim = HIDDEN_DIM, latent_dim = LATENT_DIM): 
        super(Encoder_Context, self).__init__()
        # input_dim = 4 * len_history
        #att
        ...
        self.training = True
        #print("0003>>>>>>>>>>>>>>>>>>>>>>>>>context_dim = ",input_dim)
        
    def forward(self, x):
        #print("0002>>>>>>>>>>>>>>>>>>>>>>>>>x.type =",x.type)
...
class Decoder_Context(nn.Module):
    def __init__(self, output_dim, latent_dim = LATENT_DIM, hidden_dim = HIDDEN_DIM):
        super(Decoder_Context, self).__init__()
        # output_dim = 4 * len_history
        #att
...
class Context_Model(nn.Module):
    def __init__(self, context_dim):
        ...
        self.encoder_context = Encoder_Context(context_dim)
        self.decoder_context = Decoder_Context(context_dim)
        #self.vae_output_bn = nn.BatchNorm1d(num_features=LATENT_DIM, affine=False)
    ...
    def forward(self, x):
        #print("0001>>>>>>>>>>>>>>>>>>>>>>>>>>>>>context输入维度 = ",x.size())
        mean, log_var = self.encoder_context(x)
        z = self.reparameterization(mean, torch.exp(0.5 * log_var)) # takes exponential function (log var -> var)
        #z = self.vae_output_bn(z)
        x_hat         = self.decoder_context(z)
        #print("00001=======================x_hat = ",x_hat)
...
class AttentionNet(nn.Module):
    def __init__(self, input_dim, embedding_dim, context_dim):
        ...
        self.initial_embedding_Aug = nn.Linear(input_dim+20, embedding_dim) # layer for non-end position
        self.end_embedding_Aug = nn.Linear(input_dim+20, embedding_dim) # embedding layer for end position
        
        self.next_node_embeding = nn.Linear(input_dim, embedding_dim)
        ...
        self.context_embedding = Context_Model(context_dim=context_dim)
        if USE_WAE:
            self.wae_model = WAE_Model()
        
        if USE_VAE_A:
            self.vae_model_a = VAE_Model_A()
            
        self.forward_model_wae = Forward_Model()

    if USE_VAE:
        if NEXT_OB:
            def graph_embedding(self, node_inputs, edge_inputs, pos_encoding, context_input, mask=None):
                ...
                #---------------------
                #print("001===============context_input.size() =",context_input.size())
                x_hat, mean, log_var, _ = self.context_embedding(context_input)
                #("002===============mean.size() =",mean.size())
                #print("003===============log_var.size() =",log_var.size())
                next_end_position = x_hat[:, 0, :].unsqueeze(1)
                next_embedding_feature = torch.cat(
                                (self.end_embedding(next_end_position), self.initial_embedding(x_hat[:, 1:, :])), dim=1)
                #---------------------
                embedding_feature = embedding_feature + pos_encoding + next_embedding_feature
                ...
                return embedding_feature, x_hat
        else:
            def graph_embedding(self, node_inputs, edge_inputs, pos_encoding, context_input, mask=None):
                ...
                embedding_feature = torch.cat(
                                (self.end_embedding(end_position), self.initial_embedding(node_inputs[:, 1:, :])), dim=1)
                pos_encoding = self.pos_embedding(pos_encoding)
                x_hat, mean, log_var, context_latent = self.context_embedding(context_input)
                embedding_feature = embedding_feature + pos_encoding + context_latent
        
                sample_size = embedding_feature.size()[1]
                embedding_dim = embedding_feature.size()[2]
                embedding_feature = self.encoder(embedding_feature)
                return embedding_feature, x_hat, mean, log_var
    
    elif USE_WAE:
        if Aug_S:
            def graph_embedding(self, node_inputs, edge_inputs, pos_encoding, context_input, mask=None):
                z, x_recon = self.wae_model(context_input)
                node_inputs = torch.cat((node_inputs, z), dim=-1)
                end_position = node_inputs[:, 0, :].unsqueeze(1)
                #embedding_feature = torch.cat(
                #                (self.end_embedding(end_position), self.initial_embedding(node_inputs[:, 1:, :])), dim=1)
                embedding_feature = torch.cat(
                                (self.end_embedding_Aug(end_position), self.initial_embedding_Aug(node_inputs[:, 1:, :])), dim=1)
                pos_encoding = self.pos_embedding(pos_encoding)
                ...
                embedding_dim = embedding_feature.size()[2]
                embedding_feature = self.encoder(embedding_feature)
                return embedding_feature, x_recon, z
                #return embedding_feature, x_recon, z_real, next_ob
        else:
            def graph_embedding(self, node_inputs, edge_inputs, pos_encoding, context_input, mask=None):
                print("000===============node_inputs.size() =",node_inputs.size())
                end_position = node_inputs[:, 0, :].unsqueeze(1)
                print("001===============end_position.size() =",end_position.size())
                embedding_feature = torch.cat(
                                (self.end_embedding(end_position), self.initial_embedding(node_inputs[:, 1:, :])), dim=1)
                #print("001===============embedding_feature.size() =",embedding_feature.size())
                pos_encoding = self.pos_embedding(pos_encoding)
                
                #---------------------
                #print("001===============context_input.size() =",context_input.size())
                z, x_recon = self.wae_model(context_input)
                print("002===============z.size() =",z.size())
                #z, x_recon, z_real = self.wae_model(context_input)
                z_concat = torch.cat((context_input, z), dim=2)
                #print("002===============z_concat.size() =",z_concat.size())
                next_ob = self.forward_model_wae(z_concat)
                #print("003===============next_ob.size() =",next_ob.size())
                next_end_position = next_ob[:, 0, :].unsqueeze(1)
                #print("0031===============next_end_position.size() =",next_end_position.size())
                next_end_positionFeature = self.end_embedding(next_end_position)
                #print("004===============next_end_positionFeature.size() =",next_end_positionFeature.size())
                initial_embeddingfeature = self.initial_embedding(next_ob[:, 1:, :])
                #print("005===============initial_embeddingfeature.size() =",initial_embeddingfeature.size())
                #next_embedding_feature = torch.cat(
                #                (self.end_embedding(next_end_position), self.initial_embedding(next_ob[:, 1:, :])), dim=1)
                next_embedding_feature = torch.cat((next_end_positionFeature, initial_embeddingfeature), dim=1)
                #---------------------
                ###print("005===============embedding_feature.size() =",embedding_feature.size())
                ###print("006===============pos_encoding.size() =",pos_encoding.size())
                ###print("007===============next_embedding_feature.size() =",next_embedding_feature.size())
                embedding_feature = embedding_feature + pos_encoding + next_embedding_feature
                ...
                print("005===============embedding_feature.size() =",embedding_feature.size())
                embedding_feature = self.encoder(embedding_feature)
                return embedding_feature, x_recon, z, next_ob
                #return embedding_feature, x_recon, z_real, next_ob
        ...
        def graph_embedding(self, node_inputs, edge_inputs, pos_encoding, context_input, mask=None):
            ...
            #z, x_recon, z_real = self.wae_model(context_input)
            x_recon, z, mean, log_var = self.vae_model_a(context_input)
            ...
            embedding_feature = self.encoder(embedding_feature)
            return embedding_feature, x_recon, mean, log_var, next_ob
    ...
    def select_next_node(self, embedding_feature, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c, mask):
        ...
        ##print("006===============embedding_feature.size() =",embedding_feature.size())
        ##print("007===============budget_inputs.size() =",budget_inputs.size())

        embedding_feature = self.budget_embedding(torch.cat((embedding_feature, budget_inputs, th), dim=-1))
        ##print("008===============embedding_feature.size() =",embedding_feature.size())
    ...
    if USE_VAE:
        if NEXT_OB:
            def forward(self, node_inputs, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c, pos_encoding, context_input, mask=None):
                with autocast():
                    embedding_feature, x_hat = self.graph_embedding(node_inputs, edge_inputs, pos_encoding, context_input, mask=None)
                ...
                return logp_list, value, LSTM_h, LSTM_c, x_hat
        else:
            def forward(self, node_inputs, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c, pos_encoding, context_input, mask=None):
                with autocast():
                    embedding_feature, x_hat, mean, log_var = self.graph_embedding(node_inputs, edge_inputs, pos_encoding, context_input, mask=None)
                    logp_list, value, LSTM_h, LSTM_c = self.select_next_node(embedding_feature, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c, mask)
                return logp_list, value, LSTM_h, LSTM_c, x_hat, mean, log_var
    elif USE_WAE:
        if Aug_S:
            def forward(self, node_inputs, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c, pos_encoding, context_input, mask=None):
                    with autocast():
                        embedding_feature, x_recon, z_real = self.graph_embedding(node_inputs, edge_inputs, pos_encoding, context_input, mask=None)
                        logp_list, value, LSTM_h, LSTM_c = self.select_next_node(embedding_feature, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c, mask)
                    return logp_list, value, LSTM_h, LSTM_c, x_recon, z_real
        else:
            def forward(self, node_inputs, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c, pos_encoding, context_input, mask=None):
                    with autocast():
                        embedding_feature, x_recon, z_real, next_ob = self.graph_embedding(node_inputs, edge_inputs, pos_encoding, context_input, mask=None)
                        logp_list, value, LSTM_h, LSTM_c = self.select_next_node(embedding_feature, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c, mask)
                    return logp_list, value, LSTM_h, LSTM_c, x_recon, z_real, next_ob
    elif USE_VAE_A:
        def forward(self, node_inputs, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c, pos_encoding, context_input, mask=None):
                with autocast():
                    embedding_feature, x_recon, mean, log_var, next_ob = self.graph_embedding(node_inputs, edge_inputs, pos_encoding, context_input, mask=None)
                    logp_list, value, LSTM_h, LSTM_c = self.select_next_node(embedding_feature, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c, mask)
                return logp_list, value, LSTM_h, LSTM_c, x_recon, mean, log_var, next_ob
    else:
        def forward(self, node_inputs, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c, pos_encoding, mask=None):
            with autocast():
                #print("000003===================================node_inputs.size = ", node_inputs.size())
                #embedding_feature = self.graph_embedding(node_inputs, edge_inputs, pos_encoding, context_latent, mask=None)
