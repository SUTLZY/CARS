...
class Gaussian2D():
    def __init__(self):
        self.rangeDistribs = (8,12) #(1,2)#(8,12)
        self.mean = []
        self.sigma_x = []
        self.sigma_y = []
        self.cov = []
    ...
    def addGaussian(self):
        ...
        gaussian_var[([0, 1], [0, 1])] = np.random.uniform(0.00005,0.0002,2)#(0.0000005,0.000002,2)#(0.00005,0.0002,2)
    ...
    def createProbability(self):
        ...
        # print(numDistribs)
        if RANDOM_ENV:
            for _ in range(numDistribs-1):
                self.addGaussian()
            gaussian_mean = np.random.rand(2)
            self.mean.append(gaussian_mean)
            gaussian_var = np.zeros((2, 2))
            gaussian_var[([0, 1], [0, 1])] = np.random.uniform(0.00005/3,0.0002/3,2)
            SigmaX = np.sqrt(gaussian_var[0][0])
            SigmaY = np.sqrt(gaussian_var[1][1])
            Covariance = gaussian_var[0][1]
            self.sigma_x.append(SigmaX)
