...
class GaussianProcessForIPP():
    ...
    #def update_node(self, timestep = 0):
    def update_node(self):
        y_pred, std = self.gp.predict(self.node_coords, return_std=True)
        '''
        if RANDOM_ENV:
            T = 20  # 噪音周期
            noise_amplitude = 0.5  # 噪音振幅

            # 计算当前时间步的噪音值
            noise = np.random.uniform(0, noise_amplitude * np.sin(2 * np.pi * timestep / T) + noise_amplitude, 1)
    ...
    def evaluate_cov_trace(self, X=None, timestep = 0):   # 没有拿真实reward
        if X is None:
            x1 = np.linspace(0, 1, 30)
            x2 = np.linspace(0, 1, 30)
        ...
        if RANDOM_ENV:
            T = 20  # 噪音周期
            noise_amplitude = 1  # 噪音振幅 3
            # 计算当前时间步的噪音值
            # noise = np.random.uniform(0, noise_amplitude * np.sin(2 * np.pi * timestep / T) - noise_amplitude, 1)
            noise = noise_amplitude * np.sin(2 * np.pi * timestep / T) - noise_amplitude
            # noise = -timestep * np.random.uniform(0, noise_amplitude)
