...
class Worker:
    ...
    def run_episode(self, currEpisode):
        ...
        node_info_std_inputs = np.zeros((n_nodes, 2), dtype=float)
        node_inputs_first = np.concatenate((node_coords, node_info_std_inputs), axis=1)
        node_inputs_first = torch.FloatTensor(node_inputs_first).to(self.device)
        #print("=====================================node_inputs_first.size() = ", node_inputs_first.size())
        history_buffer = [node_inputs_first] * LEN_HISTORY
        ...
        for i in range(256):
            ...
            history_buffer_input = torch.cat(history_buffer, dim=-1).unsqueeze(0)
            history_buffer_input = torch.FloatTensor(history_buffer_input).to(self.device)
            episode_buffer[13] += history_buffer_input
            history_buffer += node_inputs
            history_buffer = history_buffer[-LEN_HISTORY:]

            with torch.no_grad():
                if USE_VAE:
                    if NEXT_OB:
                        logp_list, value, LSTM_h, LSTM_c, _ = self.local_net(node_inputs, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c, pos_encoding, history_buffer_input, mask)
                    else:
                        logp_list, value, LSTM_h, LSTM_c, _, _, _ = self.local_net(node_inputs, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c, pos_encoding, history_buffer_input, mask)
                elif USE_WAE:
                    if Aug_S:            # ##
                        logp_list, value, LSTM_h, LSTM_c, _, _ = self.local_net(node_inputs, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c, pos_encoding, history_buffer_input, mask)
                    else:
                        logp_list, value, LSTM_h, LSTM_c, _, _, _ = self.local_net(node_inputs, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c, pos_encoding, history_buffer_input, mask)
                elif USE_VAE_A:
                    logp_list, value, LSTM_h, LSTM_c, _, _, _, _ = self.local_net(node_inputs, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c, pos_encoding, history_buffer_input, mask)
                else:
            ...
            with torch.no_grad():
                if USE_VAE:
                    if NEXT_OB:
                        ...
                        _, value, LSTM_h, LSTM_c, _, _, _ = self.local_net(node_inputs, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c, pos_encoding, history_buffer_input, mask)
                elif USE_WAE:
                    logp_list, value, LSTM_h, LSTM_c, _, _, _  = self.local_net(node_inputs, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c, pos_encoding, history_buffer_input, mask)
                elif USE_VAE_A:
                    logp_list, value, LSTM_h, LSTM_c, _, _, _, _  = self.local_net(node_inputs, edge_inputs, budget_inputs, current_index, LSTM_h, LSTM_c, pos_encoding, history_buffer_input, mask)
        ...
        #print("000001===================================episode_buffer[0].size() = ", episode_buffer[0].size())
        #print("000002===================================episode_buffer[13].size() = ", episode_buffer[13].size())
