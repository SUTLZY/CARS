...
from parameters import ADAPTIVE_AREA, RANDOM_ENV, STEP_SIZE, SEED
...
class Env():
    ...
    def reset(self, seed=None):
        ...
        #self.node_info, self.node_std = self.gp_ipp.update_node(self.timestep)
    ...
    def step(self, next_node_index, sample_length, measurement=True):   #采集current_node_index到next_node_index沿线所有点的value
        ...
        no_sample = True
        #self.underlying_distribution.update_ground_true()
        ...
        #F1score = self.gp_ipp.evaluate_F1score(self.ground_truth)
            ...
            self.RMSE = RMSE
        cov_trace = self.gp_ipp.evaluate_cov_trace(self.high_info_area,self.timestep)
        ...
        if RANDOM_ENV:
            # self.underlying_distribution.move_randomly(STEP_SIZE)
            # self.underlying_distribution.updateGroundtrue()
            self.ground_truth = self.get_ground_truth()
